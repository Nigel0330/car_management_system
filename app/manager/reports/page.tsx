...
export default function ManagerReportsPage() {
  ...
  const handleGenerate = async () => {
    ...
    if (svcErr || cliErr) { setError('Failed to fetch data.'); setLoading(false); return }
    ...
    rows.push(`Total Services,${services?.length ?? 0}`)
    ...
    ;(services ?? []).forEach(svc => {
      const v = svc.vehicles as unknown as Service['vehicles']
      rows.push([
        v?.clients?.full_name ?? '—',
        v?.car_model ?? '—',
        v?.plate_number ?? '—',
  ...
  return (
    <div style={{ maxWidth: '500px' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ margin: '4px 0 0', color: '#6b7280', fontSize: '13px' }}>Download a CSV report for a selected date range</p>
      </div>
      ...
      <div style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ background: '#f8fafc', borderRadius: '8px', padding: '12px', border: '1px solid #e5e7eb' }}>
          ...
          {['Summary — total clients, services, reminders sent', 'Services — all services in date range', 'Clients — full client list'].map(item => (
            <p key={item} style={{ margin: '3px 0', fontSize: '12px', color: '#6b7280' }}>• {item}</p>
          ))}
        </div>
